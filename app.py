...
.stTabs [aria-selected="true"] {
    ...
    box-shadow: 0 4px 15px rgba(222, 255, 154, 0.3) !important;
}
...
/* Custom AI Sinyal Kartları (HTML Enjeksiyonu İçin) */
.ai-signal-card {
...
.badge-bull { background: rgba(163, 255, 0, 0.1); color: #A3FF00; padding: 4px 10px; border-radius: 12px; font-weight: 700; font-size: 12px; border: 1px solid #A3FF00;}
.badge-bear { background: rgba(255, 76, 76, 0.1); color: #FF4C4C; padding: 4px 10px; border-radius: 12px; font-weight: 700; font-size: 12px; border: 1px solid #FF4C4C;}
...
st.markdown("Yapay Zeka Destekli Katılım Endeksli Yeşil Enerji Portföy Yönetim Sistemi (V10.0)")
...
try:
    ...
    with tab1:
        ...
        fig.add_trace(go.Scatter(
            ...
            mode='lines', name='Taha Yeşil Fon', line=dict(color='#DEFF9A', width=3.5),
            hovertemplate="<b>Fon:</b> %{y:.2f}<extra></extra>"
        ))

        fig.add_trace(go.Scatter(
            x=normalize_veri.index.strftime('%Y-%m-%d'), y=normalize_veri['XU100.IS'],
            mode='lines', name='BIST100 Endeksi', line=dict(color='#3B82F6', width=2), 
        ...
        if trend_goster:
            ...
            if f'SMA_{sma_uzun}' in normalize_veri.columns:
                fig.add_trace(go.Scatter(
                    ...
                    mode='lines', name=f'SMA {sma_uzun}', line=dict(color='#FF4C4C', width=1.5, dash='dot')
                ))

        fig.update_layout(
            plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)', font=dict(color='#A0ABC0'),
            xaxis=dict(showgrid=True, gridcolor='#1E2532', tickangle=-45, showline=False),
            yaxis=dict(showgrid=True, gridcolor='#1E2532', showline=False),
            legend=dict(orientation="h", yanchor="top", y=-0.15, xanchor="center", x=0.5, font=dict(color='#FFFFFF')),
            margin=dict(l=0, r=0, t=10, b=0), hovermode='x unified'
        )
        # SAKIN DOKUNMA: modebar'ı sonsuza dek sildik (Müşteri şikayeti iptali)
    ...
    with tab2:
        ...
        st.markdown("Portföydeki bireysel hisselerin derinlikli teknik analizini ve işlem hacimlerini inceleyin.")
        
        secili_hisse = st.selectbox("İncelenecek Hisseyi Seçin", ['ALFAS.IS', 'YEOTK.IS', 'ASTOR.IS', 'KCAER.IS'])
        
        if secili_hisse:
            # Sadece tek hisse verisi
            ...
            # Alt grafikler (Fiyat, Hacim, RSI, MACD)
            ...
            # Mum Grafik
            fig_tech.add_trace(go.Candlestick(x=h_close.index, open=h_open, high=h_high, low=h_low, close=h_close, 
                                              name="Fiyat", increasing_line_color='#DEFF9A', decreasing_line_color='#FF4C4C'), row=1, col=1)
            
            # Hacim
            colors_vol = ['#DEFF9A' if row['close'] >= row['open'] else '#FF4C4C' for index, row in pd.concat([h_open, h_close], axis=1, keys=['open', 'close']).iterrows()]
            fig_tech.add_trace(go.Bar(x=h_vol.index, y=h_vol, name="Hacim", marker_color=colors_vol), row=2, col=1)
            
            # RSI
            fig_tech.add_trace(go.Scatter(x=h_rsi.index, y=h_rsi, name="RSI", line=dict(color='#3B82F6', width=2)), row=3, col=1)
            ...
            # MACD
            fig_tech.add_trace(go.Scatter(x=h_macd.index, y=h_macd, name="MACD", line=dict(color='#DEFF9A')), row=4, col=1)
            fig_tech.add_trace(go.Scatter(x=h_signal.index, y=h_signal, name="Signal", line=dict(color='#FF4C4C')), row=4, col=1)
            fig_tech.add_trace(go.Bar(x=h_hist.index, y=h_hist, name="Histogram", marker_color=['#DEFF9A' if val >= 0 else '#FF4C4C' for val in h_hist]), row=4, col=1)
            
            fig_tech.update_layout(
                height=800, plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)', font=dict(color='#A0ABC0'),
                showlegend=False, xaxis_rangeslider_visible=False, margin=dict(l=0, r=0, t=30, b=0)
            )
    ...
    with tab3:
        ...
        st.markdown("Algoritmamız, şirket haberlerini analiz ederek piyasa duygu durumunu (Sentiment) çıkarır ve yatırım tavsiyesi üretir.")
        
        bull_words = ['yüksel', 'artış', 'kâr', 'büyüme', 'anlaşma', 'pozitif', 'hedef', 'rekor', 'yatırım', 'teşvik', 'up', 'surge', 'profit']
        bear_words = ['düşüş', 'zarar', 'risk', 'negatif', 'satış', 'kayıp', 'kriz', 'dava', 'ceza', 'uyarı', 'down', 'drop', 'loss', 'sell']
                    ...
                    if news and len(news) > 0:
                        for n in news[:2]: 
                            ...
                            if bull_score > bear_score:
                                badge = '<span class="badge-bull">🟢 BULLISH (POZİTİF)</span>'
        ...
        with col_ai2:
            ...
            if trend_goster and f'SMA_{sma_uzun}' in normalize_veri.columns:
                ...
                if son_fiyat > sma_d * 1.10:
                    durum, renk, taktik = "AŞIRI ALIM (ISINMA)", "#F59E0B", "Kâr Al / Nakite Geç"
                elif son_fiyat > sma_d:
                    durum, renk, taktik = "GÜÇLÜ TREND", "#A3FF00", "Pozisyonu Koru (HOLD)"
                else:
                    durum, renk, taktik = "DÜŞÜŞ / FIRSAT", "#FF4C4C", "Destek Bekle / Topla"
            else:
    ...
    with tab4:
        ...
        if len(getiriler) > 0:
            ...
            # Value at Risk (VaR) %95 Güven Aralığı
            ...
            r_col3.metric("Günlük VaR (%95 Güven)", f"%{var_95:.2f}", delta="1 Günde Beklenen Maks Kayıp", delta_color="off")
            ...
            with kor_col1:
                ...
                fig_corr.update_layout(
                    plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)', font=dict(color='#A0ABC0'),
                    margin=dict(t=20, b=20, l=20, r=20), xaxis_showgrid=False, yaxis_showgrid=False, yaxis_autorange='reversed'
                )
            ...
            with kor_col2:
                ...
                fig_dd.add_trace(go.Scatter(
                    ...
                    fill='tozeroy', mode='lines', line=dict(color='#FF4C4C', width=1.5),
                    fillcolor='rgba(255, 76, 76, 0.1)', name='Düşüş'
                ))
            ...
            st.markdown("---")
            st.markdown("### 🔮 Yapay Zeka Monte Carlo Simülasyonu (1 Yıllık Projeksiyon)")
            ...
            sim_df = np.zeros((gun_sayisi, sim_sayisi))
            sim_df[0] = baslangic
            
            for t in range(1, gun_sayisi):
                sok = np.random.normal(loc=mu, scale=sigma, size=sim_sayisi)
                sim_df[t] = sim_df[t-1] * (1 + sok)
                
            fig_mc = go.Figure()
            # Şeffaf Simülasyon Çizgileri
            for i in range(sim_sayisi):
                fig_mc.add_trace(go.Scatter(
                    y=sim_df[:, i], mode='lines',
                    line=dict(color='rgba(222, 255, 154, 0.03)', width=1), showlegend=False, hoverinfo='skip'
                ))
                
            # Ortalama Beklenen Çizgi
            beklenen = sim_df.mean(axis=1)
            fig_mc.add_trace(go.Scatter(
                y=beklenen, mode='lines', name='Beklenen Ortalama Senaryo',
                line=dict(color='#3B82F6', width=3, dash='dash')
            ))
            
            fig_mc.update_layout(
                height=500, plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)', font=dict(color='#A0ABC0'),
                xaxis=dict(showgrid=True, gridcolor='#1E2532', title="Gelecek İşlem Günleri"),
                yaxis=dict(showgrid=True, gridcolor='#1E2532', title="Sermaye Büyüklüğü (TL)"),
            ...
            son_degerler = sim_df[-1, :]
            iyi_senaryo = np.percentile(son_degerler, 95)
            kotu_senaryo = np.percentile(son_degerler, 5)
            
            mc1, mc2, mc3 = st.columns(3)
            mc1.metric("Kötü Senaryo (%5 Olasılık)", f"{kotu_senaryo:,.0f} ₺", delta=f"%{((kotu_senaryo-baslangic)/baslangic)*100:.1f}", delta_color="normal")
            mc2.metric("Beklenen Senaryo", f"{beklenen[-1]:,.0f} ₺", delta=f"%{((beklenen[-1]-baslangic)/baslangic)*100:.1f}", delta_color="normal")
            mc3.metric("İyi Senaryo (%95 Olasılık)", f"{iyi_senaryo:,.0f} ₺", delta=f"%{((iyi_senaryo-baslangic)/baslangic)*100:.1f}", delta_color="normal")

        else:
            st.warning("Veri yetersiz.")

except Exception as e:
    st.error(f"Sistem bir anormallik tespit etti. Lütfen sayfayı yenileyin. Hata: {e}")
